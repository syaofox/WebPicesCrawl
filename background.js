...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "extract") {
    extractedImages = request.data.urls;
    pageTitle = request.data.title;
    ...
    if (extractedImages.length > 0) {
      downloadImages(extractedImages, pageTitle, pageUrl).then(downloadedCount => {
        sendResponse({success: true, count: downloadedCount});
        // 发送下载完成消息到 content script
        chrome.tabs.sendMessage(currentTabId, {action: "downloadComplete", count: downloadedCount});
      }).catch(error => {
